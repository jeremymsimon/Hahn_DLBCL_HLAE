---
title: "Cindy Secondary Screen Analysis: TMD8"
...
```{r }
screen2_tmd8_stats_e <- read_excel("2023-08-04 TMD8 HLA-E Hi Lo Hypergeo.xlsx") %>%
...
```{r}
screen2_tmd8_stats_abc <- read_excel("2023-08-04 TMD8 HLA-I Hi Lo Hypergeo.xlsx") %>%
	dplyr::rename("Gene_Symbol" = `Gene Symbol`, "AverageLFC" = `Average LFC`, "AverageLogP" = `Average -log(p-values)`) %>%
	dplyr::select(Gene_Symbol,AverageLFC,AverageLogP)
screen2_tmd8_stats_abc
...
```{r }
tmd8_lfcs_e <- read_csv("2023-08-11_TMD8_HLAE_counts_lfc_gene.csv") %>%
	dplyr::select(Construct_Barcode,Gene_Symbol,pDNA,contains("ave"),contains("LFC"))

tmd8_lfcs_e_nt <- tmd8_lfcs_e %>%
...
tmd8_e_zscores <- tmd8_lfcs_e %>%
  dplyr::filter(!str_detect(Gene_Symbol,"INTERGENIC|NO_SITE")) %>%
  mutate(Z_HILO_HLAE = (LFC_Hi_Lo - tmd8_lfcs_e_nt$MeanLFC_HiLo)/tmd8_lfcs_e_nt$SDLFC_HiLo,
         Z_FLOWp_HLAE = (LFC_input_pDNA - tmd8_lfcs_e_nt$MeanLFC_FLOWp)/tmd8_lfcs_e_nt$SDLFC_FLOWp
         ) %>%
  group_by(Gene_Symbol) %>%
  mutate(nTargets = n_distinct(Construct_Barcode))

tmd8_e_zscores_genelevel <- tmd8_e_zscores %>%
  ungroup() %>%
...
tmd8_e_combined <- full_join(tmd8_e_zscores,tmd8_e_zscores_genelevel,by="Gene_Symbol")
...
```{r }
tmd8_lfcs_abc <- read_csv("2023-08-11_TMD8_HLAI_counts_lfc_gene.csv") %>%
	dplyr::select(Construct_Barcode,Gene_Symbol,pDNA,contains("ave"),contains("LFC"))

tmd8_lfcs_abc_nt <- tmd8_lfcs_abc %>%
  dplyr::filter(str_detect(Gene_Symbol,"INTERGENIC|NO_SITE")) %>%
  ungroup() %>%
  summarize(MeanLFC_HiLo = mean(LFC_Hi_Lo),
            MeanLFC_FLOWp = mean(LFC_input_pDNA),
...
tmd8_abc_zscores <- tmd8_lfcs_abc %>%
  dplyr::filter(!str_detect(Gene_Symbol,"INTERGENIC|NO_SITE")) %>%
  mutate(Z_HILO_HLAI = (LFC_Hi_Lo - tmd8_lfcs_abc_nt$MeanLFC_HiLo)/tmd8_lfcs_abc_nt$SDLFC_HiLo,
         Z_FLOWp_HLAI = (LFC_input_pDNA - tmd8_lfcs_abc_nt$MeanLFC_FLOWp)/tmd8_lfcs_abc_nt$SDLFC_FLOWp
         ) %>%
  group_by(Gene_Symbol) %>%
  mutate(nTargets = n_distinct(Construct_Barcode))

tmd8_abc_zscores_genelevel <- tmd8_abc_zscores %>%
  ungroup() %>%
  dplyr::select(-Construct_Barcode,-LFC_Hi_Lo,-LFC_input_pDNA) %>%
  group_by(Gene_Symbol) %>%
...
tmd8_abc_combined <- full_join(tmd8_abc_zscores,tmd8_abc_zscores_genelevel,by="Gene_Symbol")
...
```{r}
#| echo: false
pdf("SecondaryScreen_TMD8_HLAE_viability_scatterplot.pdf", height = 5, width = 6)
tmd8_e_combined %>%
	dplyr::select(Gene_Symbol,Z.target.HILO.HLAE,Z.target.FLOWp.HLAE,P.target.adj.HLAE,P.FLOWp.adj.HLAE) %>% 
  ungroup() %>%
  distinct() %>%
...
tmd8_combined_genelevel <- tmd8_combined %>%
...
tmd8_combined_genelevel %>%
  ...
  ylab("<b style='font-size:20pt;'>**TMD8**</b><br>HLA-I High vs Low<br>Z-score<br><br>HLA-I low←  →HLA-I high") +
...
```{r}
#| echo: false
cairo_pdf("SecondaryScreen_TMD8_HLAEvsHLAI_Zscores_specificity.pdf")
tmd8_combined_genelevel %>%
  ggplot(aes(x=Z.target.HILO.HLAE,y=Z.target.HILO.HLAI,label=Gene_Symbol,fill=HLAE_specificity)) +
  geom_point(color="gray50",pch=21,stroke=0.25) +
  geom_abline(slope=1, intercept=0, linewidth = 0.5) +
...
```{r}
tmd8_results <- tmd8_combined_genelevel %>%
  inner_join(screen2_tmd8_stats_e,by="Gene_Symbol") %>%
  dplyr::rename("HypergeoLFC_HLAE" = AverageLFC) %>%
  dplyr::rename("HypergeoLogP_HLAE" = AverageLogP) %>%
  inner_join(screen2_tmd8_stats_abc,by="Gene_Symbol") %>%
  dplyr::rename("HypergeoLFC_HLAI" = AverageLFC) %>%
  dplyr::rename("HypergeoLogP_HLAI" = AverageLogP)
  
tmd8_results

write_tsv(tmd8_results,file="SecondaryScreen_TMD8_geneLevel_compiledResults_122225.tsv")
```

## Save session

```{r}
save.image("SecondaryScreen_TMD8_analysis_122225.RData")
```
