---
title: "Cindy Secondary Screen Analysis: SUDHL4"
...
```{r }
screen2_sudhl4_stats_e <- read_excel("2023-08-04 SUDHL4 HLA-E Hi Lo Hypergeo.xlsx") %>%
...
```{r}
screen2_sudhl4_stats_abc <- read_excel("2023-08-04 SUDHL4 HLA-I Hi Lo Hypergeo.xlsx") %>%
	dplyr::rename("Gene_Symbol" = `Gene Symbol`, "AverageLFC" = `Average LFC`, "AverageLogP" = `Average -log(p-values)`) %>%
	dplyr::select(Gene_Symbol,AverageLFC,AverageLogP)
screen2_sudhl4_stats_abc
...
```{r }
sudhl4_lfcs_e <- read_csv("2023-08-11_SUDHL4_HLAE_counts_lfc_gene.csv") %>%
	dplyr::select(Construct_Barcode,Gene_Symbol,pDNA,contains("ave"),contains("LFC"))

sudhl4_lfcs_e_nt <- sudhl4_lfcs_e %>%
...
sudhl4_e_zscores <- sudhl4_lfcs_e %>%
  dplyr::filter(!str_detect(Gene_Symbol,"INTERGENIC|NO_SITE")) %>%
  mutate(Z_HILO_HLAE = (LFC_Hi_Lo - sudhl4_lfcs_e_nt$MeanLFC_HiLo)/sudhl4_lfcs_e_nt$SDLFC_HiLo,
         Z_FLOWp_HLAE = (LFC_input_pDNA - sudhl4_lfcs_e_nt$MeanLFC_FLOWp)/sudhl4_lfcs_e_nt$SDLFC_FLOWp
         ) %>%
  group_by(Gene_Symbol) %>%
  mutate(nTargets = n_distinct(Construct_Barcode))

sudhl4_e_zscores_genelevel <- sudhl4_e_zscores %>%
  ungroup() %>%
...
sudhl4_e_combined <- full_join(sudhl4_e_zscores,sudhl4_e_zscores_genelevel,by="Gene_Symbol")
...
```{r }
sudhl4_lfcs_abc <- read_csv("2023-08-11_SUDHL4_HLAI_counts_lfc_gene.csv") %>%
	dplyr::select(Construct_Barcode,Gene_Symbol,pDNA,contains("ave"),contains("LFC"))

sudhl4_lfcs_abc_nt <- sudhl4_lfcs_abc %>%
  dplyr::filter(str_detect(Gene_Symbol,"INTERGENIC|NO_SITE")) %>%
  ungroup() %>%
  summarize(MeanLFC_HiLo = mean(LFC_Hi_Lo),
            MeanLFC_FLOWp = mean(LFC_input_pDNA),
...
sudhl4_abc_zscores <- sudhl4_lfcs_abc %>%
  dplyr::filter(!str_detect(Gene_Symbol,"INTERGENIC|NO_SITE")) %>%
  mutate(Z_HILO_HLAI = (LFC_Hi_Lo - sudhl4_lfcs_abc_nt$MeanLFC_HiLo)/sudhl4_lfcs_abc_nt$SDLFC_HiLo,
         Z_FLOWp_HLAI = (LFC_input_pDNA - sudhl4_lfcs_abc_nt$MeanLFC_FLOWp)/sudhl4_lfcs_abc_nt$SDLFC_FLOWp
         ) %>%
  group_by(Gene_Symbol) %>%
  mutate(nTargets = n_distinct(Construct_Barcode))

sudhl4_abc_zscores_genelevel <- sudhl4_abc_zscores %>%
  ungroup() %>%
  dplyr::select(-Construct_Barcode,-LFC_Hi_Lo,-LFC_input_pDNA) %>%
  group_by(Gene_Symbol) %>%
...
sudhl4_abc_combined <- full_join(sudhl4_abc_zscores,sudhl4_abc_zscores_genelevel,by="Gene_Symbol")
...
```{r}
#| echo: false
pdf("SecondaryScreen_SUDHL4_HLAE_viability_scatterplot.pdf", height = 5, width = 6)
sudhl4_e_combined %>%
	dplyr::select(Gene_Symbol,Z.target.HILO.HLAE,Z.target.FLOWp.HLAE,P.target.adj.HLAE,P.FLOWp.adj.HLAE) %>% 
  ungroup() %>%
  distinct() %>%
...
```{r}
sudhl4_combined <- sudhl4_e_combined %>%
  ...
  inner_join(sudhl4_abc_combined %>% 
...
sudhl4_combined_genelevel <- sudhl4_combined %>%
...
sudhl4_combined_genelevel %>%
  ...
  ylab("<b style='font-size:20pt;'>**SU-DHL-4**</b><br>HLA-I High vs Low<br>Z-score<br><br>HLA-I low←  →HLA-I high") +
  ...
  geom_text_repel(color="black",
		data = sudhl4_combined_genelevel %>%
...
```{r}
#| echo: false
cairo_pdf("SecondaryScreen_SUDHL4_HLAEvsHLAI_Zscores_specificity.pdf")
sudhl4_combined_genelevel %>%
  ggplot(aes(x=Z.target.HILO.HLAE,y=Z.target.HILO.HLAI,label=Gene_Symbol,fill=HLAE_specificity)) +
  geom_point(color="gray50",pch=21,stroke=0.25) +
  geom_abline(slope=1, intercept=0, linewidth = 0.5) +
...
```{r}
sudhl4_results <- sudhl4_combined_genelevel %>%
  inner_join(screen2_sudhl4_stats_e,by="Gene_Symbol") %>%
  dplyr::rename("HypergeoLFC_HLAE" = AverageLFC) %>%
  dplyr::rename("HypergeoLogP_HLAE" = AverageLogP) %>%
  inner_join(screen2_sudhl4_stats_abc,by="Gene_Symbol") %>%
  dplyr::rename("HypergeoLFC_HLAI" = AverageLFC) %>%
  dplyr::rename("HypergeoLogP_HLAI" = AverageLogP)
  
sudhl4_results

write_tsv(sudhl4_results,file="SecondaryScreen_SUDHL4_geneLevel_compiledResults_122225.tsv")
```

## Save session

```{r}
save.image("SecondaryScreen_SUDHL4_analysis_122225.RData")
```
